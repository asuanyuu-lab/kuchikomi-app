...
st.title("🏨 ホテル口コミ分析 - 【2026年最新モデル対応版】")
...
if analyze_btn and target_input:
    ...
    # 2026年現在の最新安定モデルをハードコード（自動検知での事故を防ぐ）
    # もしこれでもダメなら 'gemini-1.5-flash' に書き換えます
    ACTIVE_MODEL = "gemini-3-flash"

    with st.status("🔍 口コミ抽出中...", expanded=True) as status:
    ...
    st.info(f"🤖 最新AI ({ACTIVE_MODEL}) で分析中...")
